 ...
 *
 * ### Key Flow
 * 1. 지정된 언어(lang)와 토픽(topic)으로 Wikipedia API 엔드포인트를 구성합니다.
...
class WikipediaSearchToolImpl @Inject constructor(
    ...
    override suspend fun search(topic: String, lang: String): AppResult<String> = withContext(Dispatchers.IO) {
        try {
            val baseUrl = "https://$lang.wikipedia.org/w/api.php"
            val searchUrl = "$baseUrl?action=query&format=json&generator=search&gsrsearch=$topic&gsrlimit=1&prop=extracts&explaintext=1&exintro=1&origin=*"
            ...
            if (!searchJson.has("query") || !searchJson.getJSONObject("query").has("pages")) {
                return@withContext AppResult.Failure(com.kosmos.app.core.common.AppError.NetworkUnavailable("No Wikipedia articles found matching '$topic' in language '$lang'."))
            }
            ...
            val maxChars = when (lang.lowercase()) {
                "zh" -> 1500
                "fr" -> 4300
                "es" -> 4500
                else -> 5000
