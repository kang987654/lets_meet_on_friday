...
object Constants {
    ...
    //
    // [WHY] `EngineConfig.maxNumTokens` 를 일부러 넘기지 않는다. 그 값은 엔진 KV 캐시 크기를
    // 정하고 초기화 시점에만 읽히므로, 설정 슬라이더로 바꾸려면 엔진을 다시 올려야 한다(7~11초).
    // 또 KV 크기를 우리가 임의로 정하면 지금 동작하는 런타임 기본값보다 **작게** 잡을 위험이
    // 있다(gallery 의 기본값은 1024 다). 대신 이 예산으로 히스토리 윈도우와 대화 재설정
    // 임계값을 통제한다 — 둘 다 KV 성장의 실질적 지배 요인이다.
    ...
    // [WHY] 런타임이 살아 있는 Conversation 을 버리고 새로 만드는 하한 임계값이다. 임계값
    // 자체는 사용자 설정(프리필 예산)에서 파생되지만, 예산 최소값(1000)을 그대로 쓰면
    // 시스템 지시 + 툴 선언(~2천 토큰)만으로 이미 넘어서서 **매 턴 재생성**된다. 재생성은
    // 전체 프리필이라 우리가 없애려는 바로 그 비용이므로 하한을 둔다
    // (PREFILL_OVERHEAD_TOKENS 에 여유를 더한 값).
    const val MIN_CONVERSATION_RESET_TOKENS = 4000
