...
기존 세부 항목
- [ ] **툴 실행 도중의 중첩 추론** — `GetScheduleToolExecutor` 가 `SummarizeScheduleUseCase` 를 호출하므로 채팅에서 "오늘 일정 알려줘" 하면 툴 실행 중에 추론이 한 번 더 돌고 그 산문이 모델에게 돌아간다. 구조화된 일정 목록을 돌려주면 이 추론이 사라지지만, **모델이 보는 툴 결과가 바뀌어** 툴 호출 실패(ADR-016 확정 3) 조사에 변수가 늘어난다 — 그 조사가 끝난 뒤에 다룬다 (사용자 결정, 0.12.0)
...
### 2026-08-12 실기기 검증 결과 (완료)

`adb` 로 앱 DB·감사 로그·logcat 을 직접 읽어 판정했다. **통과 6건**: 프리필 절약(`conversation created` 3턴에 1번, 5.8 → 15~18 자/초), 일회성 경로가 채팅 대화를 안 깸(캘린더 왕복 후 16.3 자/초), 상태 스트립(`🌡 35.4°C · RAM 8.4/10.9GB(앱 5.5GB) · 9.6 tok/s`), 마이크 권한 거부 스낵바, `Tools declared:` 진단, 전사 실패 시 빈 메시지 미저장.

**실패 5건** → 3건 수정, 2건 원인 규명 후 결정 대기 (0.12.0, ADR-016).

이후 검증은 **`scratch/lab/` 실험실에서 한다** — 실기기 DB 를 픽스처로 보관해 기기 없이 재현한다.

**사용자 실기기가 필요한 것 (남은 것)**
- **v0.12.0 음성 실기기 최종 확인** — PC 실험(exp16)에서 전사·무음 모두 검증했으나 **사람 목소리**는 TTS 합성본으로 대체했다. 확인할 것: ① 마이크로 말하고 내가 말한 문장이 그대로 뜨는지, ② 무음 녹음 → "음성을 인식하지 못했어요" 안내가 뜨고 빈 메시지도 지시문(`"이 오디오를 들리는 그대로…"`)도 저장되지 않는지
- **v0.12.0 캘린더 안내·요약** — 기기 캘린더 권한을 거부한 채 일정 탭 → 안내 카드가 뜨는지(예전에는 일정 0건일 때만 안 떴다). 일정이 있으면 목록이 **즉시** 뜨고 요약이 뒤이어 붙는지
- **글자 유실 재확인 (미해결)** — `"다음주 월요일 10시 팀 회의"` → 승인 카드의 시각이 `10:00` 으로 온전한지. 2026-08-12 에는 `2026-08-17T01:000` 이었다. 원인 후보가 AAR 0.14.0 런타임과 KV 용량 초과로 좁혀졌으므로(ADR-016), 그중 하나를 손본 뒤 이 한 줄로 판정한다
- **툴 호출 재확인 (0.13.0 에서 수정)** — 히스토리가 쌓인 대화에서 `"위키에서 ○○ 검색해줘"`, `"다음주 월요일 10시 팀 회의"`, `"내 자전거 비밀번호 뭐였지?"` → 감사에 `TOOL_CALL` 이 턴마다 남는지. 원인은 지침과 발화 사이의 거리였고 사용자 턴 앞에 지침 한 줄을 재게시해 해소했다(ADR-017). 로컬에서 원본 실기기 히스토리로 **6/6** 확인했으므로 실기기는 최종 확인용이다
- **설정 슬라이더 상한 변경 확인 (0.13.0)** — 설정 → 대화 기억 범위 상한이 3000 으로 내려갔는지. 기존에 6000·8000 을 저장한 설치라면 읽을 때 잘리므로 슬라이더가 3000 에 붙어 보이는 것이 정상이다
