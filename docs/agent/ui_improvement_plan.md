...
**사용자 실기기가 필요한 것**
- **v0.11.0 음성·상태 표시 확인 (최우선)** — ① 마이크로 말하고 채팅에 **내가 말한 문장이 그대로** 뜨는지(`"(음성 메시지)"` 가 아님), ② 무음 상태로 녹음 종료 → "음성을 인식하지 못했어요" 안내가 뜨고 빈 메시지가 저장되지 않는지, ③ 상단 상태 줄의 tok/s 가 생성 중 갱신되고 발열 시 떨어지는지, ④ **캘린더 화면을 열었다가 채팅으로 돌아와도** 두 번째 질문이 여전히 빠른지(일회성 경로가 대화를 안 깨는지), ⑤ 마이크·캘린더 권한을 거부했을 때 안내가 뜨는지, ⑥ 웹 검색을 켜고 비행기 모드에서 질문 → '웹 검색 보강 실패' 안내가 뜨는지
- **v0.10.0 기억 조회 확인** — 매 턴 자동 RAG 주입을 없애고 `SearchMemory` 툴로 옮겼다(ADR-013). ① `"내 자전거 비밀번호는 1234야, 기억해줘"` 로 저장한 뒤 ② `"내 자전거 비밀번호 뭐였지?"` 로 물어 정확히 나오는지, ③ 감사 로그에 `TOOL_CALL / Tool: SearchMemory` 가 남는지. 저장한 적 없는 것을 물으면(예: `"내 여권번호 뭐였지?"`) **지어내지 않고 없다고 답하는지**가 특히 중요하다
- **v0.9.1 글자 유실·툴 호출 확인 (최우선)** — 스트리밍 채널의 토큰 유실을 무한 버퍼로 막았다. 확인할 것: ① 위키 검색 답변에서 **글자가 빠지지 않는지**("2015년 10월 20일"이 온전한지), ② **2턴 이후에도 검색이 실행되는지**(감사 로그에 `TOOL_CALL`·`SEARCH_USED` 가 턴마다 남는지). ②가 여전히 실패하면 감사의 `MODEL_RUN` 항목에 새로 찍히는 **`Tools declared:`** 줄을 봐 달라 — `SearchWikipedia` 가 목록에 있는데 `TOOL_CALL` 이 없으면 모델의 거부(프롬프트 문제), 목록에 없으면 토글·allowlist 문제다
- **v0.9.0 응답 속도 체감** — 시스템 지시를 세션 고정으로 만들어 매 턴 전체 프리필을 없앴다(ADR-010). PC 에서는 2번째 턴이 3.8초 → 0.5초였다. 확인할 것: **같은 대화에서 두 번째·세 번째 질문이 첫 질문보다 눈에 띄게 빨라지는지.** 빨라지지 않으면 다른 조건(툴 목록 변경, 예산 초과)으로 대화가 재생성되고 있다는 뜻이므로 `GemmaModelRunner` 의 `conversation created` 로그가 매 턴 찍히는지 본다(디버그 빌드에서만 찍힌다)
