...
**사용자 실기기가 필요한 것**
- **v0.8.7 일정·조회 실기기 확인 (최우선)** — 툴 호출 자체는 0.8.6 에서 관통 확인됨(승인 카드 → 실행 → 저장). PC 실험(ADR-009)에서 5/5 로 고친 것들의 실기기 확인이 남았다: ① `"내일 3시에 치과 예약 잡아줘"` → 승인 카드에 `2026-08-08T15:00` 이 찍히는지(되묻지 않는지), ② `"다음주 월요일 10시 팀 회의"` → 날짜가 한 주 밀리지 않는지, ③ `"내일 스케줄 알려줘"` → 오늘이 아닌 주간 일정이 나오는지
