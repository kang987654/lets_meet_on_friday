...
처방 후보: ① 툴 호출을 DB 에 저장해 히스토리에 함께 재생(Room v5→v6), ② 히스토리에 싣는 어시스턴트 메시지를 짧게 요약, ③ 툴을 쓴 턴만 재구성. **프롬프트 문구 수정은 처방이 아니다** — 이미 "MUST call" 수준으로 못박혀 있고 명시 지시도 무시됐다.
...
처방은 용량을 올리거나(KV 메모리 증가) 예산을 낮추거나(문맥 축소), 둘을 한 값에서 파생시켜 어긋날 수 없게 만드는 것이다. 셋 다 대가가 있어 사용자 결정이 필요하다.
...
**남은 1순위**: AAR 0.14.0 과 파이썬 0.15.0 의 런타임 차이. 확정 4(용량 초과)와의 상호작용도 후보다 — 0.14.0 이 초과분을 조용히 버린다면 그것이 곧 글자 유실일 수 있다. 버전이 달라 이 하네스로는 가를 수 없다.
