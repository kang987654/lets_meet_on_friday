...
### ADR-022. 세션은 저장 개념이다 — 에피소드 기억과 시안 A′ 채택 (2026-08-15)

- **Status**: Accepted (사용자 결정)
- **Context**: UI 전면 재구성 논의에서 "비서의 본체는 대화"라는 합의 위에, 세션 관리 UI(expand.md
  A1)를 설계하려다 원칙 충돌이 드러났다 — 자비스형 비서와의 상호작용에는 세션이 없다.

#### 결정

**세션은 UX 에 노출하지 않는다.** 사용자에게는 하나의 연속 대화(무한 스크롤 타임라인)만 보이고,
뒤에서 대화가 에피소드로 자동 분절(무활동 30분 + 예산 리셋 경계)되어 oneShot 요약(제목/태그/
요약)이 검색 가능한 문서로 저장된다. 과거는 히스토리 윈도우가 아니라 **검색(SearchMemory ·
드로어 검색)**으로 돌아오고, 회수 사용은 답변에 칩(🧠)으로 표시한다. 상세 설계와 목업은
`docs/design/ui_a_prime.md`.

#### 근거 (exp33, 실기기 픽스처 79개)

분절: 9 에피소드 전부 주제 일관. 요약: oneShot 형식 준수 9/9. 회수: 모델 검색어 확장 + 문자
바이그램(FTS5 대용)으로 **recall@1 = 16/16** — 바꿔 말하기(자물쇠↔비밀번호, 지어냄↔환각) 포함,
임베딩 없이. 개인 1인 규모(에피소드 수백 개)에서는 검색 기법보다 **저장 품질(요약·태그)**이
병목이라는 판단을 뒷받침한다. C3(임베딩) 게이트는 유지 — 부족 사례가 기록될 때만 연다.

#### 결과

- expand.md A1(세션 관리 UI)은 이 결정으로 **대체**된다 — 만들 것은 세션 목록이 아니라 기억
  아카이브다. C4(대화 자동 지식화)는 에피소드 요약 파이프라인에 흡수된다.
